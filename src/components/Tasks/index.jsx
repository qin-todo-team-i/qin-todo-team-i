...
export const Tasks = ({ limit }) => {
  ...
  const [filterdTaskAddGroup, setFilterdTaskAddGrooup] = useState([]);
  ...
  useEffect(() => {
    ...
    setFilterdTaskAddGrooup(_filterdTaskAddGroup);
    console.log({ filterdTaskAddGroup });
  }, [tasks]);

  const moveItem = useCallback(
    (dragIndex, targetIndex, group) => {
      console.log("moveItem", dragIndex, targetIndex, group);
      const item = filterdTaskAddGroup[dragIndex];
      if (!item) return;
      setFilterdTaskAddGrooup((prevState) => {
        const newItems = prevState.filter((_, idx) => idx !== dragIndex);
        newItems.splice(targetIndex, 0, { ...item, group });
        return newItems;
      });
    },
    [filterdTaskAddGroup, setFilterdTaskAddGrooup]
  );
  ...
  return (
    <div className="w-full p-5">
      ...
      <div className="mt-6 space-y-2">
        {filterdTaskAddGroup
          ? filterdTaskAddGroup.map((task, index) =>
              targetId === task.id ? (
                // タスク編集状態
                <div key={task.id}>
